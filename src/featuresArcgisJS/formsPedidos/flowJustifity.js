...
async function verifyAprovacao(polygonGraphics, update, projetos) {
  ...
  const promises = polygonGraphics.map(async graphic => {
    ...
    const [featureData1, featureData2] = await Promise.all([
      retornaListAreaCode(projetos, true, update.Projeto, 1),
      retornaListAreaCode(projetos, true, update.Projeto, 2)
    ]);
