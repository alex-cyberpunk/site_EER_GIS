...
async function connectAPPG(){
    if(!client)
        client = new MongoClient(process.env.MONGO_CONNECTION); 
    await client.connect();//so pode utilizar dentro de funcoes async
    return client.db(process.env.DATABASEAPPG); 
}
...
async function connectLogs(){
    if(!client)
        client = new MongoClient(process.env.MONGO_CONNECTION); 
    await client.connect();//so pode utilizar dentro de funcoes async
    return client.db(process.env.DATABASELOGS); 
}
...
module.exports ={connect,disconnect,connectAPPG,connectLogs}